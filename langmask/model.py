...
class MultilingualLanguageModel:
    ...
    def __init__(
        ...
        self.tokenizer: PreTrainedTokenizer = AutoTokenizer.from_pretrained(model_name, **_tokenizer_kwargs)

        logger.info(f"Loading model: {model_name} onto device: {self._device}")
        self.model: PreTrainedModel = AutoModelForCausalLM.from_pretrained(model_name, **_model_kwargs).to(self._device)
    ...
    def generate(
        self,
        prompt: str,
        max_length: Optional[int] = None, # Default behavior often depends on model config
        ...
        **kwargs: Any # Passthrough for other generate() arguments
        ...
        if max_length is not None:
             generate_kwargs.pop("max_new_tokens", None) # Remove max_new_tokens if max_length is set

...
class MultilingualTokenMasker:
    ...
    def __init__(
        ...
        logger.info(f"Allowed languages: {', '.join([f'{lang} ({SUPPORTED_LANGUAGES[lang]})' for lang in self.allowed_languages])}")
        ...
        for attr in special_token_attrs:
            if hasattr(self.tokenizer, attr):
                ...
                if token:
                    ...
                    if token_id is not None and token_id != self.tokenizer.unk_token_id or token == self.tokenizer.unk_token:
    ...
    def apply_language_mask(self, logits: torch.Tensor) -> torch.Tensor:
        ...
        if len(valid_allowed_ids) != len(allowed_token_ids):
             logger.warning("Some allowed token IDs are outside the tokenizer's vocabulary size.")
             
        disallowed_mask = torch.ones(vocab_size, device=self.device, dtype=torch.float32)
        ...
        if self.mask_strength >= 0.9999: # Use a threshold for hard masking
            ...
            penalty = penalty_factor * torch.log(torch.tensor(1.0 - self.mask_strength + 1e-9, device=self.device)) 
    ...
    def set_token_threshold(self, threshold: float) -> None:
         ...
         self._token_cache = {}
         logger.info("Token cache cleared due to threshold change. Will re-cache on next use.")
    ...
    def set_allowed_languages(self, languages: List[str]) -> None:
        ...
        logger.info(f"Allowed languages set to: {', '.join([f'{lang} ({SUPPORTED_LANGUAGES[lang]})' for lang in self.allowed_languages])}")
    ...
    def add_language(self, language: str) -> bool:
        ...
        logger.info(f"Language {language} ({SUPPORTED_LANGUAGES[language]}) added to allowed list.")
    ...
    def remove_language(self, language: str) -> bool:
        ...
        if len(self.allowed_languages) <= 1:
            logger.warning("Cannot remove the last allowed language. At least one language must be allowed.")
            return False
            
        self.allowed_languages.remove(language)
        logger.info(f"Language {language} ({SUPPORTED_LANGUAGES[language]}) removed from allowed list.")
        # Note: We don't remove from cache, as it might be added back later.
        return True

    # --- Debugging Methods ---

    def debug_token_classification(self, text: str, verbose: bool = True) -> Dict[str, int]:
        """
        ...
        for token_str, token_id in zip(tokens, token_ids):
            ...
            if token_str.startswith('Ġ'): decoded_token = token_str[1:]
            elif token_str.startswith(' '): decoded_token = token_str[1:]
            elif token_str.startswith('##'): decoded_token = token_str[2:]
            ...
            if decoded_token.isspace() or decoded_token.isdigit() or (len(decoded_token) == 1 and not decoded_token.isalnum()):
                 classification["NEUTRAL"].append(token_str)
                 continue

            classified = False
            for lang_code in SUPPORTED_LANGUAGES:
                if is_language_token(token_str, lang_code, threshold=self.token_threshold):
        ...
        if verbose:
            ...
            for lang_code, count in stats.items():
                if count > 0:
                    ...
                    lang_name = SUPPORTED_LANGUAGES.get(lang_code, lang_code) # Use code if not in map (e.g., SPECIAL)
                    print(f"  {lang_code} ({lang_name}): {count} tokens ({percentage:.1f}%)")
            print(f"------------------------------------")
